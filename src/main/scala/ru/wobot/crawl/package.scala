...
package object crawl {
  ...
  case class SuccessFetch[T](uri: String, crawlDate: Long, content: T) extends Fetch

  trait Parsed

  case class SuccessParsed[T](uri: String, content: T) extends Parsed

  trait Parser {
    def parse(uri: String, content: String): Parsed
  ...
  case class Post(url: String, date: String, profileName: String, profileUrl: String, body: String, likes: Long, reposts: Long, comments: Long, title: Option[String], city: Option[String])
